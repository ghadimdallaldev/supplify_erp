...
| chat_realtime      | Real-time chat & alerts  | MULTI      | —                                  | —             | apps/api/src/lib/socket.js (JWT cookie auth, room join authz); optional Redis adapter (`socket-redis-adapter.js`) | ChatPage (`useChatRealtime`), Layout (`useNotificationAlerts`); `getAppSocket()` | All plans with `chat`; `notification_new` for in-app toasts |
...
| delivery_gps_tracking | Delivery GPS (supplier + restaurant) | MULTI      | ORDERS_VIEW / FULFILLMENT_VIEW | —         | `orders-driver.routes.js` `GET /:id/tracking`; `driver-location.service.js`; restaurant sanitized via `restaurant-tracking-payload.js` | FulfillmentPage, RestaurantOrderTrackingPanel, DriverDeliveriesPage, SupplierCommandCenterPage | Gold+ drivers; restaurant when `GPS_ALLOW_RESTAURANT_LIVE_TRACKING`       |
...
| product_csv_import   | Bulk product CSV import   | SUPPLIER   | CATALOG_EDIT         | storage_mb (image_url) | supplier-ops.routes.js POST `/products/import/*`; optional `image_url` via product-import.service.js                                     | ProductBulkUploadDialog  | Sync import; partial rows OK      |
| product_image_import | Bulk image ZIP import     | SUPPLIER   | CATALOG_EDIT         | storage_mb             | supplier-ops.routes.js POST `/products/images/import/*`; async job `catalog_image_import_job` (migration 0168)                           | ProductImageImportDialog | Preview → job poll; web-only v1   |
...
| staff_self_service | Staff portal (STAFF_PORTAL + `/api/staff/self/*`; magic link legacy) | STAFF_PORTAL / public token | —                    | —         | staff-portal-auth.js, staff.routes.js `/self`, public.routes.js                                               | StaffSelfServiceLogin.tsx, StaffPortalGuard, StaffSelfServiceDashboard.tsx | IMPLEMENTED       |
| staff_portal_admin | Manager portal account controls on Team tab                          | RESTAURANT                  | STAFF_EDIT           | —         | staff.routes.js `/members/:id/portal/*`                                                                       | StaffPortalAccessPanel.tsx                                                 | IMPLEMENTED       |
...
| admin_impersonation    | Impersonate tenant (full workspace)                                                                                | ADMIN      | ADMIN_ACCESS         | —         | impersonation.js, impersonationContext.js, impersonation-guards.js; admin-dashboard POST/GET /impersonate, /stop; rbac getRequestTenant + requireRole bypass | ImpersonationBanner, useImpersonation, AdminDashboardPage Tenants; BranchContext; tenant pages (Orders, Products, Settings, …)                                                                     | —                 |
...
- Dashboard is impersonation-aware: when user is ADMIN and not impersonating, DashboardPage shows an admin landing (no order calendar, no tenant stats); tenant content and Order Calendar only when effective role is RESTAURANT or SUPPLIER (including when admin is impersonating). See [features/admin-impersonation.md](../features/admin-impersonation.md).
...
| smart_reorder         | Smart reorder (feature flag)       | RESTAURANT | INVENTORY_VIEW       | —                   | `requireFeature('smart_reorder')` on reorder-assistance/forecasts/ai-recommend; tier via `resolveSmartReorderCapabilities()` | ReorderAssistancePanel.tsx, Dashboard widget    | Restaurant Growth+ forecasts; Scale adds NL ask (`ai_forecast_seasonality`) |
| ai_platform           | AI platform (LLM assist)           | RESTAURANT | —                    | ai_requests_per_day | `isAiPlatformEnabledForTenant` on explain/ask/ai-recommend; admin + plan override                                            | ReorderAssistancePanel.tsx                      | Growth/Scale when plan + quota allow; trial uses AI pool                    |
...
| ai_requests_per_day       | RESTAURANT | reorder-ai.service.js (`checkAndIncrementUsage` on genuine LLM explain/ask/recommend)       |
...
| free_trial_expiry_lock     | Free Trial expiry (read-only)       | MULTI      | —                    | —           | `billingAccessMiddleware`; `free-sandbox-expiry.job`; `buildAccountLockedError`; admin `extend-free-trial` / unlock extends `free_sandbox_expires_at`        | BillingOverdueBanner, SubscriptionInfo, AdminPlatformSettingsPanel, Extend trial      | Plan `free` only; days 7–90 (default 30)   |
| supplier_customer_growth   | Import, invite, sponsor, referrals  | SUPPLIER   | CUSTOMERS\_\*        | GROWTH_VIEW | `supplier-growth.routes.js`; `referral-conversion.service.js`; migration `0169`                                                                              | SupplierCustomerGrowthPage, DashboardWidgetGrid, AdminGrowthSettingsPanel             | Web-first; see supplier-customer-growth.md |
