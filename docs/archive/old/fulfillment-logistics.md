...
| `GPS_LOCATION_RETENTION_DAYS`        | `90`     | Daily cron deletes old rows from `driver_location_ping` (see [CRON_JOBS.md](../operations/cron-jobs.md)) |

Full list: [environment-variables.md](../operations/environment-variables.md) · templates in `apps/api/.env.example`. Implementation state: [drivers-and-gps-tracking.md](./drivers-and-gps-tracking.md).
...
Regression IDs in [regression-checklist.md](../qa/regression-checklist.md): **§7.4.1** (supplier GPS), **§6.6.1** (restaurant GPS), **DRV-\*** (driver portal). Demo path: [../archive/old/demo-script.md](../qa/../archive/old/demo-script.md) §3.
