...
Use `pnpm memory:measure` and `GET /health` memory fields (see commands below). Historical memory audit notes live in [archive/audits](../archive/audits/).

For subscription vs RBAC alignment, see **[access-control.md](../architecture/access-control.md)**.
