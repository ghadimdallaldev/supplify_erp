...
6. **Restaurant `notifyLowStock()`** is defined but **never called**.
...
| S9  | **BACKEND_RULE_MISMATCH**                 | `notification.service.js`                       | `notifyLowStock()` never invoked for restaurants                                        | Call on restaurant inventory adjust below threshold          | **Low**                                   | Wire on PATCH/adjust routes or remove dead code                 |
