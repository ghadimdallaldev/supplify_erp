...
- **Script:** `node scripts/migrate-users-to-roles.js` (idempotent) — seeds system roles per tenant and assigns roles from legacy permissions (Owner fallback when needed).
