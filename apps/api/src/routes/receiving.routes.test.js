import express from 'express'
import request from 'supertest'
import { beforeEach, describe, expect, it, vi } from 'vitest'

vi.mock('../lib/db.js', () => {
  const queryMock = vi.fn()
  return {
    query: queryMock,
    withTransaction: vi.fn(async (fn) => fn({ query: queryMock })),
    pool: { query: queryMock },
    __queryMock: queryMock,
  }
})

vi.mock('../lib/rbac.js', () => ({
  requireAuth: (req, res, next) => next(),
  requireRole: () => (req, res, next) => next(),
  resolveTenantContext: (req, res, next) => {
    req.tenantContext = {
      tenantId: 'rest-1',
      tenantType: 'RESTAURANT',
      permissions: ['RECEIVING_VIEW'],
    }
    req.userData = { id: 'user-1', email: 'owner@resto.com', role: 'RESTAURANT' }
    next()
  },
  requirePermission: () => (req, res, next) => next(),
  getRestaurantIdForRequest: vi.fn().mockResolvedValue('rest-1'),
  getSupplierIdForRequest: vi.fn().mockResolvedValue('sup-1'),
}))

vi.mock('../lib/subscription.js', () => ({
  requireFeature: () => (req, res, next) => next(),
}))

vi.mock('../lib/logger.js', () => ({
  logger: { info: vi.fn(), error: vi.fn(), warn: vi.fn(), debug: vi.fn() },
}))

vi.mock('../services/invoice.service.js', () => ({
  assertNoReceivingReport: vi.fn(),
  createInvoiceFromReceiving: vi.fn().mockResolvedValue(null),
  lockOrderForReceiving: vi.fn().mockResolvedValue({ id: 'order-1', status: 'DELIVERED' }),
}))

vi.mock('../services/reviews.service.js', () => ({
  notifyLeaveReviewIfEligible: vi.fn().mockResolvedValue(undefined),
}))

vi.mock('../services/notification.service.js', () => ({
  notifyInvoiceIssued: vi.fn().mockResolvedValue(undefined),
}))

vi.mock('../services/inventory-expiry.service.js', () => ({
  createLotFromReceivingLine: vi.fn().mockResolvedValue(undefined),
}))

vi.mock('../services/loyalty.service.js', () => ({
  earnLoyaltyOnOrderReceive: vi.fn().mockResolvedValue(null),
}))

vi.mock('../services/recipe-purchasing-hooks.service.js', () => ({
  hookRecipeCostingAfterReceiving: vi.fn(),
  hookRecipeCostingAfterInvoice: vi.fn(),
}))

vi.mock('../services/reorder-forecast-cache.service.js', () => ({
  markReorderForecastDirty: vi.fn().mockResolvedValue(undefined),
}))

import { receivingRoutes } from './receiving.routes.js'

describe('receiving.routes pending-orders', () => {
  let app
  let db

  beforeEach(async () => {
    const mod = await import('../lib/db.js')
    db = mod
    db.query.mockReset()
    app = express()
    app.use((req, res, next) => {
      req.requestId = 'test'
      next()
    })
    app.use('/api/receiving', receivingRoutes)
  })

  it('includes DELIVERED orders in pending query', async () => {
    db.query
      .mockResolvedValueOnce({ rows: [{ id: 'order-delivered' }] })
      .mockResolvedValueOnce({ rows: [] })

    const res = await request(app).get('/api/receiving/pending-orders').expect(200)

    expect(res.body.data.orders).toHaveLength(1)
    const sql = db.query.mock.calls[0][0]
    const params = db.query.mock.calls[0][1]
    expect(sql).toMatch(/status::text = ANY/)
    expect(params[1]).toContain('DELIVERED')
    expect(params[1]).toContain('COMPLETED')
    expect(params[0]).toBe('rest-1')
    expect(sql).not.toMatch(/FROM restaurant WHERE[\s\S]*contact_email/)
    expect(sql).not.toMatch(/contact_email/)
  })
})

describe('receiving.routes pending-orders/supplier', () => {
  let app
  let db

  beforeEach(async () => {
    const mod = await import('../lib/db.js')
    db = mod
    db.query.mockReset()
    app = express()
    app.use((req, res, next) => {
      req.requestId = 'test'
      req.userData = { id: 'user-s', email: 'supplier@test.com', role: 'SUPPLIER' }
      next()
    })
    app.use('/api/receiving', receivingRoutes)
  })

  it('includes DELIVERED and legacy COMPLETED in supplier pending query', async () => {
    db.query.mockResolvedValueOnce({ rows: [{ id: 'order-legacy' }] })

    const res = await request(app).get('/api/receiving/pending-orders/supplier').expect(200)

    expect(res.body.data.orders).toHaveLength(1)
    const params = db.query.mock.calls[0][1]
    expect(params[1]).toContain('DELIVERED')
    expect(params[1]).toContain('COMPLETED')
    expect(db.query.mock.calls[0][0]).not.toMatch(/contact_email/)
  })
})

describe('receiving.routes POST /receive line completeness', () => {
  let app
  let db

  beforeEach(async () => {
    const mod = await import('../lib/db.js')
    db = mod
    db.query.mockReset()
    db.withTransaction.mockClear()
    app = express()
    app.use(express.json())
    app.use((req, res, next) => {
      req.requestId = 'test'
      next()
    })
    app.use('/api/receiving', receivingRoutes)
  })

  it('rejects partial line payloads before creating a report', async () => {
    db.query.mockResolvedValueOnce({
      rows: [
        {
          id: 'oi-1',
          quantity: 10,
          product_id: 'p-1',
          unit_price: 5,
          supplier_id: 'sup-1',
          unit: 'kg',
        },
        {
          id: 'oi-2',
          quantity: 4,
          product_id: 'p-2',
          unit_price: 12,
          supplier_id: 'sup-1',
          unit: 'case',
        },
      ],
    })

    const res = await request(app)
      .post('/api/receiving/receive')
      .send({
        orderId: 'order-1',
        lineItems: [
          {
            orderItemId: 'oi-1',
            productId: 'p-1',
            received_quantity: 10,
            quality_status: 'ACCEPTED',
          },
        ],
      })
      .expect(400)

    expect(res.body.error.name).toBe('VALIDATION_ERROR')
    expect(res.body.error.message).toMatch(/missing/i)
    expect(db.withTransaction).not.toHaveBeenCalled()
  })

  it('rejects unknown order item ids', async () => {
    db.query.mockResolvedValueOnce({
      rows: [
        {
          id: 'oi-1',
          quantity: 10,
          product_id: 'p-1',
          unit_price: 5,
          supplier_id: 'sup-1',
          unit: 'kg',
        },
      ],
    })

    const res = await request(app)
      .post('/api/receiving/receive')
      .send({
        orderId: 'order-1',
        lineItems: [
          {
            orderItemId: 'oi-1',
            productId: 'p-1',
            received_quantity: 10,
            quality_status: 'ACCEPTED',
          },
          {
            orderItemId: 'oi-unknown',
            productId: 'p-9',
            received_quantity: 1,
            quality_status: 'ACCEPTED',
          },
        ],
      })
      .expect(400)

    expect(res.body.error.message).toMatch(/unknown order item/i)
  })
})
