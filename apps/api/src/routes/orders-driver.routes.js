...
router.post('/:id/assign-driver', requirePermission('FULFILLMENT_MANAGE'), async (req, res) => {
  try {
...
router.patch('/:id/delivery-status', requirePermission('FULFILLMENT_MANAGE'), async (req, res) => {
  try {
    const supplierId = await resolveSupplierId(req)
    if (!supplierId) {
      return res.status(403).json({
...
router.post('/:id/reassign-driver', requirePermission('FULFILLMENT_MANAGE'), async (req, res) => {
  try {
    const supplierId = await resolveSupplierId(req)
    if (!supplierId) {
      return res.status(403).json({
...
router.post('/:id/proof-of-delivery', requirePermission('FULFILLMENT_MANAGE'), async (req, res) => {
  try {
    const supplierId = await resolveSupplierId(req)
    if (!supplierId) {
      return res.status(403).json({
...
router.get('/:id/proof-of-delivery', requirePermission('FULFILLMENT_VIEW'), async (req, res) => {
  try {
