...
describe('Prices Routes', () => {
  ...
  describe('GET /api/prices/product/:productId', () => {
    it('should return product prices', async () => {
      ...
      const response = await request(app).get('/api/prices/product/prod-1').expect(200)
  ...
  describe('POST /api/prices', () => {
    it('should create a price', async () => {
      ...
      db.query
        .mockResolvedValueOnce({
          rows: [
            { id: productId, supplier_id: 'supplier-1', contact_email: 'supplier@example.com' },
          ], // Product ownership check
        })
        .mockResolvedValueOnce({
          rows: [{ id: 'price-1', product_id: productId, amount: 10.5, currency: 'USD' }], // INSERT price
        })
