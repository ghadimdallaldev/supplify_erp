...
router.get('/', requireAuth, requireRole(['SUPPLIER', 'ADMIN']), async (req, res) => {
  try {
    ...
    if (suppliers.length === 0 && req.userData.role !== 'ADMIN') {
    ...
    const supplierId = req.userData.role === 'ADMIN' 
...
router.put('/:id', requireAuth, requireRole(['SUPPLIER', 'ADMIN']), async (req, res) => {
  try {
    ...
    const { name, address, capacity, contact_name, contact_email, contact_phone, is_active } = req.body;
