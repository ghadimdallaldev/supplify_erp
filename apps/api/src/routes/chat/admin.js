...
const supportAdminAccess = [
  ...
  requireRole(['ADMIN']),
  resolveAdminContext,
  requirePermission(P.ADMIN_SUPPORT),
]

router.get('/admin/conversations', ...supportAdminAccess, async (req, res, next) => {
  try {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!conversations[0].is_admin_conversation) {
...
router.post('/admin/start-conversation', ...supportAdminAccess, async (req, res, next) => {
  try {
    const { tenant_id, tenant_type, initial_message } = req.body
    if (!tenant_id || !tenant_type) throw new ValidationError('tenant_id and tenant_type required')
