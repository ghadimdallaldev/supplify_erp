...
/**
 * Idempotent quote-request DDL — runs after SQL migrations so RFQ works even if
 * schema_migrations tracking drifted or a prior migration file failed partway.
 */
export async function ensureQuoteRequestSchema() {
  await query(`
  ...
  logger.info('Quote request schema ensure completed')
}
