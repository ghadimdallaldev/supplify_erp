...
export async function executeScheduledOrders() {
  try {
    ...
    logger.info(`Server time: ${todayDate} ${currentTime} (Local timezone: ${Intl.DateTimeFormat().resolvedOptions().timeZone})`);
    ...
    if (scheduledLists.length === 0) {
      ...
      if (allScheduled.length > 0) {
        ...
        allScheduled.forEach(q => {
          const execDate = q.next_execution_date ? new Date(q.next_execution_date).toISOString().split('T')[0] : 'null';
          logger.info(`  - ${q.name}: next_execution_date=${execDate} (DB: ${q.next_execution_date}), preferred_time=${q.preferred_time}, status=${q.status}`);
          logger.info(`    Comparing: todayDate=${todayDate} <= ${execDate} = ${todayDate <= execDate}`);
          if (execDate === todayDate && q.preferred_time) {
            logger.info(`    Time check: ${q.preferred_time} <= ${currentTime} = ${q.preferred_time <= currentTime}`);
          }
        });
      }
    ...
    for (const quickList of scheduledLists) {
      try {
        ...
        if (quickList.auto_create_order) {
          ...
          logger.info(`Sending reminder for quick list ${quickList.id} (auto_create_order is false)`);
        }
...
async function createOrderFromQuickList(quickList) {
  ...
  for (const [supplierId, supplierItems] of supplierGroups) {
    ...
    logger.info(`Created order ${order.id} from quick list ${quickList.id} for supplier ${supplierId}`);
  }
