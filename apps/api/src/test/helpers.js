...
export const createMockApp = (routes, options = {}) => {
  ...
  // Add 404 handler
  app.use((req, res, next) => {
    const { NotFoundError } = require('../middlewares/errorHandler.js');
...
export const getMockWithTransaction = () => mockStorage?.withTransaction || vi.fn((handler) => handler({ query: vi.fn() }));
