...
export async function runGrowthProgramMaintenanceJob() {
  const [sponsorship, invitations, connections] = await Promise.all([
    runSponsorshipMaintenanceJob().catch(async (err) => {
      logger.warn('Sponsorship maintenance failed, falling back to expiry', {
        error: err.message,
      })
      return runSponsorshipExpiryJob()
    }),
