...
testFiles.forEach((testFile) => {
  ...
  // Fix import
  const importPattern = /const \{ .*Routes \} = await import\(.*\);[\s\S]*?app\.use\(.*\);?/
  if (info.export === 'default') {
    content = content.replace(
      importPattern,
      `const routesModule = await import('./${routeFile}');\nconst ${info.name} = routesModule.default || routesModule.${info.name};\napp.use('/api/${info.path}', ${info.name});`
    )
  } else {
    content = content.replace(
      importPattern,
      `const { ${info.export} } = await import('./${routeFile}');\napp.use('/api/${info.path}', ${info.export});`
    )
  }

  // Fix route paths
  const routeName = routeFile.replace('.routes.js', '').replace(/-/g, '')
  const wrongPath = `/api/${routeName.charAt(0).toUpperCase() + routeName.slice(1)}`
  content = content.replace(
