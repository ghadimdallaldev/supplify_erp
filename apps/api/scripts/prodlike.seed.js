...
import { toDateString, toTimestamp, addDays, randomDateBetween, todayStart } from './seed/timeUtils.js';
...
const SUBSCRIPTION_STATUSES = ['TRIALING', 'ACTIVE', 'ACTIVE', 'ACTIVE', 'ACTIVE', 'PAST_DUE', 'CANCELLED'];
const BILLING_CYCLES = ['MONTHLY', 'MONTHLY', 'YEARLY'];
const INVOICE_STATUSES = ['ISSUED', 'PARTIALLY_PAID', 'PAID', 'PAID', 'PAID', 'PAID', 'PAID', 'PAID', 'OVERDUE', 'VOID'];
const RESERVATION_STATUSES = ['PENDING', 'CONFIRMED', 'CONFIRMED', 'SEATED', 'COMPLETED', 'COMPLETED', 'CANCELLED', 'WAITLIST'];
const RESTAURANT_STAFF_ROLES = ['manager', 'cashier', 'chef', 'receiver', 'accountant', 'waiter', 'waiter', 'waiter'];
...
async function runReset(client) {
  ...
  const tables = [
    ...
    ['staff_pto_request', 'staff_availability', 'staff_announcement_ack', 'staff_document', 'staff_incident', 'staff_performance_note', 'staff_payroll_export'],
    ['staff_member'],
...
async function main() {
  ...
  try {
    ...
    await bulkInsert(client, {
      table: 'supplier',
      columns: ['id', 'name', 'slug', 'vat_no', 'contact_email', 'phone', 'address_json', 'created_at', 'updated_at'],
      rows: supplierRows,
    });
    ...
    for (const s of suppliers) {
      ...
      for (let i = 0; i < n; i++) {
        const id = uuid();
        whs.push({ id, name: (n === 1 ? 'Main' : `WH-${i + 1}`) + ' Warehouse', code: 'WH' + (i + 1) });
      }
      warehouseBySupplier.set(s.id, whs);
      for (const wh of whs) {
        await client.query(
          `INSERT INTO warehouse (id, supplier_id, name, code, address_json, is_main, is_active, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, true, NOW(), NOW())`,
          [wh.id, s.id, wh.name, wh.code, JSON.stringify({ city: 'City', country: 'US' }), wh.code === 'WH1' ? true : false]
        );
        warehouseCount++;
      }
    }
    ...
    const categories = ['Produce', 'Dairy', 'Meat', 'Beverages', 'Dry Goods', 'Frozen', 'Cleaning', 'Paper'];
    ...
    await bulkInsert(client, {
      table: 'restaurant',
      columns: ['id', 'name', 'slug', 'trade_license_no', 'contact_email', 'phone', 'address_json', 'created_at', 'updated_at'],
      rows: restaurantRows,
    });
    const { rows: restaurants } = await client.query('SELECT id, name FROM restaurant ORDER BY name');
    ...
    for (const r of restaurants) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        branchIds.push(id);
        await client.query(
          `INSERT INTO branch (id, restaurant_id, name, is_active, created_at, updated_at) VALUES ($1, $2, $3, true, NOW(), NOW())`,
          [id, r.id, (n === 1 ? 'Main' : `Branch ${i + 1}`)]
        );
        branchCount++;
      }
    ...
    for (const r of restaurants) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        await client.query(
          `INSERT INTO staff_member (id, restaurant_id, status, first_name, last_name, email, role, created_at, updated_at)
           VALUES ($1, $2, 'ACTIVE', $3, $4, $5, $6, NOW(), NOW())`,
          [id, r.id, first, last, `staff-${r.id.slice(0, 8)}-${i}@test.com`, role]
        );
        staffCount++;
      }
    ...
    for (const r of restaurants) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        await client.query(
          ...
          [uuid(), r.id, branchId, status, `${first} ${last}`, '+1555' + intBetween(rng, 100000, 999999), partySize, scheduledAt.toISOString()]
        );
        resCount++;
      }
    }
    ...
    const orderStatuses = ['COMPLETED', 'COMPLETED', 'COMPLETED', 'COMPLETED', 'SHIPPED', 'PROCESSING', 'ACKNOWLEDGED', 'PLACED', 'CANCELLED'];
    ...
    for (const r of restaurants) {
      ...
      for (let o = 0; o < numOrders; o++) {
        ...
        await client.query(
          ...
          [orderId, r.id, status, status !== 'DRAFT' && status !== 'CANCELLED' ? placedAt.toISOString() : null, placedAt.toISOString()]
        );
        ...
        for (const sup of chosenSuppliers) {
          ...
          const linesForSup = intBetween(rng, 1, Math.max(1, Math.floor(numLines / numSuppliersForOrder) + 1));
          shuffle(rng, prods);
        ...
        await client.query(`UPDATE customer_order SET total_amount = $1 WHERE id = $2`, [orderTotal.toFixed(3), orderId]);
        ordersByRestaurant.get(r.id).push({ id: orderId, restaurant_id: r.id, total: orderTotal, placed_at: placedAt, status });
        orderCount++;
      }
    }
    ...
    for (const r of restaurants) {
      ...
      for (const s of missing.slice(0, 5)) {
        ...
        await client.query(`UPDATE customer_order SET total_amount = $1 WHERE id = $2`, [lineTotal.toFixed(3), orderId]);
        ordersByRestaurant.get(r.id).push({ id: orderId, restaurant_id: r.id, total: lineTotal, placed_at: placedAt, status: 'COMPLETED' });
        orderCount++;
        covered.add(s.id);
      }
    }
    ...
    for (const r of restaurants) {
      ...
      const completedOrders = restaurantOrders.filter((o) => o.status === 'COMPLETED' || o.status === 'SHIPPED');
      for (let i = 0; i < numInv; i++) {
        ...
        else if (status === 'PARTIALLY_PAID') paidAmount = Math.round(totalAmount * floatBetween(rng, 0.2, 0.8) * 100) / 100;
        ...
        if (paidAmount > 0) {
          ...
          const payDate = status === 'PAID' ? (balanceDue <= 0 ? invDate : addDays(new Date(invDate), intBetween(rng, 1, 30))) : new Date(invDate);
          await client.query(
            ...
            [payId, invoiceId, payNum, toDateString(payDate), paidAmount.toFixed(2), toTimestamp(payDate)]
          );
        }
        invoiceCount++;
      }
      ...
      for (let d = 0; d < 15; d++) {
        ...
        for (const [startsAt, endsAt] of shiftSlots) {
          ...
          await client.query(
            ...
            [uuid(), r.id, staffId, role, toDateString(shiftDate), toTimestamp(startsAt), toTimestamp(endsAt)]
          );
          shiftCount++;
        }
      }
