  ...
  return (
    <div className="relative">
      <select
        ...
        className={cn(
          'flex h-10 w-full appearance-none rounded-lg border border-[var(--app-border-mid)] bg-[var(--surface)] px-3 py-2 pr-10 text-sm text-[var(--text)] file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-[var(--text-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-mid)]/30 focus-visible:border-[var(--brand-mid)] disabled:cursor-not-allowed disabled:opacity-50',
          className
        )}
        {...props}
      >
      ...
      <ChevronDown className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 pointer-events-none text-[var(--text-muted)]" />
    </div>
  )
})
