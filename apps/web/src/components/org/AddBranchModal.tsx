...
export function AddBranchModal({ open, onClose }: Props) {
  ...
  const [createInvitation, { isLoading: inviting }] = useCreateBranchInvitationMutation()
  const { data: rolesData } = useGetBranchInviteRolesQuery(
    { supplier_id: branchId! },
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-lg">
        {step === 1 ? (
          <>
            <DialogHeader>
              <DialogTitle>Add branch account</DialogTitle>
            </DialogHeader>
            <form
              className="space-y-3"
              onSubmit={(e) => {
            ...
            >
              ...
              <FormActions onCancel={onClose} submitLabel="Create Branch" loading={creating} />
            </form>
          </>
        ) : (
          <>
            <DialogHeader>
              <DialogTitle>Branch created — invite a manager</DialogTitle>
            </DialogHeader>
