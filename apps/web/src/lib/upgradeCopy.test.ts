...
describe('upgradeCopy', () => {
  it('includes supplier deals and promotion limit copy (GATE-R19, GATE-S13, PLN)', () => {
    expect(LIMIT_UPGRADE_COPY.supplier_deals?.plan).toBe('Bronze')
    expect(LIMIT_UPGRADE_COPY.deal_redemptions_per_day?.plan).toBe('Bronze')
    expect(LIMIT_UPGRADE_COPY.promotions?.plan).toBe('Bronze')
    expect(LIMIT_UPGRADE_COPY.deal_redemptions_per_day?.value).toContain('deal')
  })
  ...
  it('getFeatureUpgradeCopy returns chat and reports entries', () => {
    expect(getFeatureUpgradeCopy('chat')?.plan).toBe('Bronze')
