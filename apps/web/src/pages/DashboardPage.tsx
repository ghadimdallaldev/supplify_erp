...
export function DashboardPage() {
  ...
  // Recent orders preview (shared)
  const { data: recentOrders } = useGetOrdersQuery({ limit: 5, offset: 0 })
  // Orders dataset for charts
  const { data: chartOrders } = useGetOrdersQuery({ limit: 50, offset: 0 })
  ...
  const { data: productCategories } = useGetProductCategoriesQuery(undefined, { skip: user?.role !== 'SUPPLIER' })
  ...
  if (isSupplier) {
    kpis.push(
      { title: 'Products', value: stats?.totalProducts ?? 0, icon: Package, description: 'Active SKUs' },
      { title: 'Pending Orders', value: stats?.pendingOrders ?? 0, icon: TrendingUp, description: 'Awaiting fulfillment' },
      { title: 'Completed Orders', value: stats?.completedOrders ?? 0, icon: ClipboardCheck, description: 'Shipped & delivered' },
    )
    if (typeof stats?.totalRevenue === 'number') {
      kpis.push({ title: 'Revenue', value: `$${Number(stats.totalRevenue).toLocaleString()}` , icon: DollarSign, description: 'All-time' })
    }
    if (typeof stats?.totalRestaurants === 'number') {
      kpis.push({ title: 'Restaurants', value: stats.totalRestaurants, icon: Users, description: 'Customers' })
    }
  }

  if (isRestaurant) {
    kpis.push(
      { title: 'Orders', value: stats?.totalOrders ?? 0, icon: ShoppingCart, description: 'All orders' },
      { title: 'Pending Orders', value: stats?.pendingOrders ?? 0, icon: TrendingUp, description: 'In progress' },
      { title: 'Completed Orders', value: stats?.completedOrders ?? 0, icon: Package, description: 'Received' },
    )
    if (typeof stats?.totalSpent === 'number') {
      kpis.push({ title: 'Total Spent', value: `$${stats.totalSpent.toFixed(2)}`, icon: DollarSign, description: 'All-time' })
    }
    if (typeof stats?.totalSuppliers === 'number') {
      kpis.push({ title: 'Suppliers', value: stats.totalSuppliers, icon: Building2, description: 'Active vendors' })
    }
  }
  ...
  if (!kpis.length) {
    kpis.push(
      { title: 'Products', value: stats?.totalProducts ?? 0, icon: Package, description: 'Available' },
  ...
  const ordersTrendData = (chartOrders?.orders || []).slice().reverse().map((o: any, i: number) => ({
    name: `#${i + 1}`,
    amount: Number(o.total_amount) || 0,
  }))
  ...
  const spendTrend = Array.isArray(invoiceAnalytics?.points)
    ? invoiceAnalytics.points.map((p: any) => ({ name: p.date?.slice(5) || '', value: Number(p.total) || 0 }))
    : []

  // Supplier product category distribution
  const categoryDist = Array.isArray(productCategories?.categories)
    ? productCategories.categories.slice(0, 8).map((c: any) => ({ name: c.name, value: c.display_order || 1 }))
    : []

  return (
    <div className="space-y-6">
      <div className="rounded-2xl p-6 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white shadow-sm">
        <h1 className="text-3xl font-bold">{isSupplier ? 'Supplier Dashboard' : isRestaurant ? 'Restaurant Dashboard' : 'Dashboard'}</h1>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {hasRevenue && (
              <div className="rounded-xl bg-white/10 p-4">
                ...
                <p className="text-lg font-semibold">${Number(stats.totalRevenue).toLocaleString()}</p>
              </div>
            )}
            {hasSpend && (
              <div className="rounded-xl bg-white/10 p-4">
