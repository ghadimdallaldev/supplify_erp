...
export function StaffSelfServiceDashboard() {
  ...
  const handleSubmitPto = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || error?.data?.error?.message || 'Unable to submit PTO request')
    }
  }
  ...
  const handleSubmitSwap = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || error?.data?.error?.message || 'Unable to submit shift swap request')
    }
  }
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Tap a shift to request a swap or mark your availability.</CardDescription>
            </CardHeader>
                ...
                upcomingShifts.map((shift: StaffPortalDashboard['upcomingShifts'][number]) => (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-xl border px-4 py-3 text-sm transition ${
                      swapForm.shiftId === shift.id ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200'
                    }`}
                  >
                    <div>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Managers will review and respond in the staff app.</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <form className="space-y-3" onSubmit={handleSubmitSwap}>
                  ...
                  <Button type="submit" className="w-full" disabled={submittingSwap || !swapForm.shiftId}>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Stay updated with important messages from leadership.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {documents.length === 0 ? (
                <p className="text-sm text-gray-500">You’re all caught up! No documents assigned right now.</p>
              ) : (
                documents.map((document: StaffPortalDashboard['documents'][number]) => (
                  <div key={document.id} className="flex items-center justify-between rounded-xl border border-gray-200 p-4">
                    <div>
                      <p className="font-medium text-gray-900">{document.title ?? document.doc_type}</p>
                      <p className="text-xs text-gray-500">
            ...
            <CardContent className="space-y-3">
              {ptoRequests.length === 0 ? (
                ...
                ptoRequests.map((request: StaffPortalDashboard['ptoRequests'][number]) => (
                  <div key={request.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-4 py-3 text-sm">
                    <div>
            ...
            <CardContent className="space-y-3">
              {swapRequests.length === 0 ? (
                ...
                swapRequests.map((swap: StaffPortalDashboard['swapRequests'][number]) => (
                  <div key={swap.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-4 py-3 text-sm">
                    <div>
                      <p className="font-medium text-gray-900">{swap.reason || 'Shift swap request'}</p>
                      <p className="text-xs text-gray-500">
