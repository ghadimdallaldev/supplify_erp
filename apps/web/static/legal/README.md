...
**Current pack version:** see repo-root [`legal-pack-version.json`](../../../../legal-pack-version.json) (imported by both `apps/web/src/lib/legalDocuments.ts` and `apps/api/src/lib/legal-documents.js`).
...
1. `TERMS_AND_CONDITIONS.md` — Master platform terms for restaurants, suppliers, staff portal users, staff, drivers, admins, public guests, reservations, **consumer guest orders**, B2B orders, branches, warehouses, subscriptions, **deals & boosts**, mobile app usage, and platform liability.
...
9. `MOBILE_APP_TERMS.md` — Native mobile app and driver/PWA-specific terms covering push, device permissions, camera, background location for drivers, offline use, app stores, and updates.
...
2. Bump `version` in repo-root `legal-pack-version.json` when users must re-accept (material legal change).
3. Deploy **API and Web together** (both read the same JSON file).
