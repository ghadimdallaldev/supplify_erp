/**
 * Split ProductsPage into components/products/
 * Run: node scripts/split-wave2.mjs products
 */
import fs from 'fs'
...
const read = (p) => fs.readFileSync(src(p), 'utf8').split(/\r?\n/)
...
function wrapDialog(name, body, imports, propsType, propsDestruct) {
  const dedented = body
    .replace(/^\{?\/\* Inventory Adjustment Dialog \*\/\}\s*\n/, '')
    .replace(/^        /gm, '')
    .replace(/^          /gm, '  ')
  return `${imports}
...
export function ${name}(${propsDestruct}) {
  return (
${dedented.split('\n').map((l) => (l ? '    ' + l : '')).join('\n')}
  )
}
...
function splitProducts() {
  const lines = read('apps/web/src/pages/ProductsPage.tsx')
  const getLines = (a, b) => lines.slice(a - 1, b)
  const slice = (a, b) => getLines(a, b).join('\n')

  const formImports = `import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
...
import type { ProductFormState } from './productsShared'`
  ...
  write(
    'apps/web/src/components/products/ProductFormDialog.tsx',
    wrapDialog(
      'ProductFormDialog',
      slice(973, 1210),
      formImports,
      `type ProductFormDialogProps = ${formProps}`,
      `{ showAddProduct, setShowAddProduct, productForm, setProductForm, newTag, setNewTag, categoriesData, tagsData, warehousesData, imagePreview, handleImageSelect, handleSubmitProduct, isCreating, isUploadingImage }: ProductFormDialogProps`
    )
  )

  const bulkImports = `import { Download } from 'lucide-react'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { downloadExampleFile } from './productsShared'`
  ...
  write(
    'apps/web/src/components/products/ProductBulkUploadDialog.tsx',
    wrapDialog(
  ...
  isCreating: boolean
      ...
      `{ showBulkUpload, setShowBulkUpload, uploadedFile, setUploadedFile, uploadPreview, setUploadPreview, importPreviewMeta, importErrors, importSummary, handleFileUpload, downloadErrorReport, handleBulkSubmit, importing, isCreating }: ProductBulkUploadDialogProps`
    )
  )

  const invImports = `import { TrendingUp, TrendingDown } from 'lucide-react'
...
import { useCreateInventoryAdjustmentMutation } from '../../services/api'`
  ...
  write(
    'apps/web/src/components/products/InventoryAdjustmentDialog.tsx',
...
`
  ...
  // ProductFilters - search is in page, filters grid + tags + active chips in catalog
  write(
...
`
  ...
  write(
    'apps/web/src/components/products/ProductCatalogTable.tsx',
...
${getLines(689, 959)
  .map((l) => {
    let l2 = l.replace(/^          /, '      ')
    l2 = l2.replace(/handleAddToCart\(/g, 'onAddToCart(')
    l2 = l2.replace(
      /setSelectedProductForAdjustment\(product\)\s*\n\s*setShowInventoryAdjustment\(true\)/g,
      'onAdjustStock(product)'
    )
    l2 = l2.replace(
      /\{\s*\n\s*setSelectedProductForAdjustment\(product\)\s*\n\s*setShowInventoryAdjustment\(true\)\s*\n\s*\}/g,
      'onAdjustStock(product)'
    )
    return l2
  })
...
if (cmd === 'products') splitProducts()
else console.log('Usage: node scripts/split-wave2.mjs products')
