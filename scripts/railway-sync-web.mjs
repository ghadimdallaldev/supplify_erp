#!/usr/bin/env node
/**
 * Push deploy/railway/<env>/web.env to the Railway web service.
 * Usage: node scripts/railway-sync-web.mjs preprod --service supplify-web-preprod
 */
...
const DEFAULT_SERVICES = {
  development: 'supplify-web-dev',
  preprod: 'supplify-web-preprod',
  staging: 'supplify-web-preprod',
  production: 'supplify-web-prod',
};

function usage() {
  console.error('Usage: node scripts/railway-sync-web.mjs <env> [--service <name>]');
  process.exit(1);
}
...
function main() {
  ...
  const envFile = resolve(root, `deploy/railway/${env}/web.env`);
